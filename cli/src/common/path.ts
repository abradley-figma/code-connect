 ...
 */
export function normalizeResolvePath(root: string, ...segments: string[]): string {
  return normalizePath(resolve(root, ...segments))
}
 ...
 */
export function normalizePathsToAbsolute(absPath: string, paths: string[]): string[] {
  return paths.map((path) => normalizePath(resolve(absPath, path)))
}
