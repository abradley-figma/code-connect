...
/**
 * Local Vite-plugin shape. ONLY the fields `figmaCodeConnect()` reads or writes.
 * Structurally compatible with `import('vite').Plugin`.
 */
...
interface ViteResolvedConfig {
  root: string
  /** `'serve'` when running `vite` / `vite serve`, `'build'` for `vite build`. */
  command?: 'serve' | 'build'
}

interface ViteModuleNode {
  // We never inspect this — Vite returns it from `getModuleById` and we hand
  // it right back via `handleHotUpdate`'s return value.
  __vite_module?: never
}

interface ViteModuleGraph {
  getModuleById(id: string): ViteModuleNode | null | undefined
}
...
interface VitePlugin {
  ...
  buildStart?(this: VitePluginContext): Promise<void>
  resolveId?(id: string): string | undefined
  load?(id: string): Promise<string | undefined>
  handleHotUpdate?(ctx: ViteHmrContext): Promise<ViteModuleNode[] | undefined> | undefined
}

export function figmaCodeConnect(opts: CodeConnectViteOptions = {}): { name: string } {
  ...
  const codeConnectCompiler = createCompiler(opts)
  ...
  let enabled: boolean = opts.enabled ?? true
  ...
  const plugin: VitePlugin = {
    ...
    configResolved(config) {
      ...
      if (!enabled) return
      if (!opts.root) codeConnectCompiler.setRoot(config.root)
    },
    ...
    resolveId(id) {
      ...
      if (id === virtualModuleId || id === runtimeModuleId) return resolvedId
      return undefined
    },
    ...
    async handleHotUpdate(ctx) {
      ...
      if (!updateResult.changed) return undefined
