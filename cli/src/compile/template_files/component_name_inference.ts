 ...
 */
export function inferComponentName(args: {
  ...
  if (args.filePath) return basenameComponent(args.filePath)
  return undefined
}

function pickFromDirective(directive?: string): string | undefined {
  ...
  if (IDENT_RE.test(candidate)) return candidate
  return undefined
}

function basenameComponent(filePath: string): string | undefined {
