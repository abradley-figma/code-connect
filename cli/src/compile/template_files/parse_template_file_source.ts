...
import { buildMockFigma } from './figma_code_connect'
import type { CaptureRecord } from './figma_code_connect'
import { executeTemplate } from './execute_template'
import { extractJsxInfo } from './extract_jsx_info'
import { inferComponentName } from './component_name_inference'
import { resolveComponentSourcePath } from './resolve_component_source'
 ...
 */
export function parseComponentDescriptorsFromSource(
  source: string,
  filePath: string = 'template.figma.ts',
  timeoutMs: number = 300,
  root: string = nodePath.dirname(filePath),
): ParseResult {
  ...
  const { js, diagnostics } = transpileSource(source, filePath)
  for (const d of diagnostics) warnings.push(`transpile: ${d}`)

  const { figma, jsxRuntime, captures } = buildMockFigma()

  const exec = executeTemplate({
    js,
    filePath,
    figma,
    jsxRuntime,
  ...
  if (exec.timedOut) {
    warnings.push(
      `${filePath}: template execution exceeded ${timeoutMs}ms ` +
  ...
  if (exec.unknownImports.length > 0) {
    warnings.push(
      `${filePath}: template imported unsupported modules: ${[
  ...
  for (const c of captures) {
    ...
    if (c.droppedReason) {
      warnings.push(
        `${filePath}: getEnum('${c.figmaPropName}') has no usable options ` +
        `(${c.droppedReason}). The panel will render an empty enum.`,
      )
    }
    if (c.skippedOptionLabels.length > 0) {
      warnings.push(
        `${filePath}: getEnum('${c.figmaPropName}') dropped ${c.skippedOptionLabels.length} ` +
  ...
  const attrToToken = new Map<string, symbol>()
  if (exec.figmaCode) {
    const jsx = extractJsxInfo(exec.figmaCode)
    rootTag = jsx.rootTag
    rootIsComponent = jsx.rootIsComponent
    for (const [k, v] of jsx.attrToToken) attrToToken.set(k, v)
  } else if (!exec.threw && !exec.timedOut) {
    warnings.push(
      `${filePath}: template's default export is not the result of figma.code\`...\`. ` +
  ...
  if (!componentName) {
    warnings.push(
      `${filePath}: could not infer component name from directive, root JSX tag, ` +
  ...
  const seenFigmaProps = new Map<string, CaptureRecord['kind']>()
  ...
  const tokenToAttr = new Map<symbol, string>()
  for (const [attr, sym] of attrToToken) {
    if (!tokenToAttr.has(sym)) tokenToAttr.set(sym, attr)
  }
  ...
  for (const c of captures) {
    ...
    if (seenKind !== undefined) {
      if (seenKind !== c.kind) {
        ...
        warnings.push(
          `${filePath}: figma-side prop name '${c.figmaPropName}' captured ` +
 ...
 * removed".
 */
export async function parseComponentDescriptorsFromFile(
  filePath: string,
  timeoutMs: number,
  root?: string,
 ...
 *  - `slot`      → `slot`
 */
function buildPropDescriptor(c: CaptureRecord, name: string): PropDescriptor | undefined {
  const base = {
    name,
 ...
 */
function captureKindToDescriptorType(kind: CaptureRecord['kind']): string {
  return kind === 'text' ? 'string' : kind
}

function isLegacyConnectFile(source: string): boolean {
...
export function extractMetadata(source: string): RawMetadata {
  ...
  for (const line of source.split('\n')) {
    ...
    if (!ALLOWED_DIRECTIVES.has(field)) continue
    if (field === 'url' && out.url === undefined) out.url = value
