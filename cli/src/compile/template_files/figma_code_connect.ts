...
export type CaptureRecord =
  | { token: symbol; kind: 'text'; figmaPropName: string }
  | { token: symbol; kind: 'boolean'; figmaPropName: string }
  | {
    token: symbol
    kind: 'enum'
  ...
  | { token: symbol; kind: 'reference'; figmaPropName: string }
  | { token: symbol; kind: 'slot'; figmaPropName: string }
...
export const JSX_STUB_MARKER = Symbol.for('@figma/code-connect/jsx-stub')

/** Marker on objects returned by `figma.code`/`tsx`/`html`/etc. so `extract_jsx_info` can detect them. */
export const FIGMA_CODE_MARKER = Symbol.for('@figma/code-connect/figma-code')

/** Marker on objects returned by `figma.value`. */
export const FIGMA_VALUE_MARKER = Symbol.for('@figma/code-connect/figma-value')
 ...
 * `attr={…}` position.
 */
export const RENDER_PROP_MARKER = Symbol.for('@figma/code-connect/render-prop')
...
export function readRenderPropMarker(value: unknown): RenderPropMarker | undefined {
  if (value && typeof value === 'object' && (value as any)[RENDER_PROP_MARKER] === true) {
    return value as RenderPropMarker
  }
  return undefined
}
...
const TOKEN_META = new WeakMap<object, TokenMeta>()

/** Read the underlying TokenMeta from a token. Used by `extract_jsx_info` via symbol identity. */
export function readTokenMeta(value: unknown): TokenMeta | undefined {
  ...
  if (value && (typeof value === 'object' || typeof value === 'function')) {
    return TOKEN_META.get(value as object)
  }
  return undefined
}
...
export function isFigmaCodeResult(v: unknown): v is FigmaCodeResult {
  return !!v && typeof v === 'object' && (v as any)[FIGMA_CODE_MARKER] === true
}
 ...
 */
function parseEnumOptions(mapping: unknown): {
  options: EnumOption[]
  skippedOptionLabels: string[]
  droppedReason?: string
} {
 ...
 * render those shapes.
 */
function coerceEnumValue(raw: unknown): string | undefined {
 ...
 */
function makeToken(meta: { kind: CaptureKind; figmaPropName: string }): {
  token: symbol
  proxy: object
} {
  ...
  const proxy: any = new Proxy(function tokenProxy() { }, {
    get(_target, prop) {
      ...
      if (prop === 'then') return undefined // don't accidentally look thenable to await/promise chains
 ...
 */
const chainStub: any = new Proxy(function chainStubFn() { }, {
  get(_t, prop) {
    if (prop === Symbol.toPrimitive) return () => ''
    if (prop === 'toString') return () => ''
    if (prop === 'then') return undefined
    if (prop === 'constructor') return undefined
    if (prop === 'example') return Object.freeze([])
    if (prop === 'metadata') return Object.create(null)
    return chainStub
  },
  getPrototypeOf() {
    return null
 ...
 */
export function buildMockFigma(): FigmaCodeConnectBuild {
  const captures: CaptureRecord[] = []

  function record<R extends CaptureRecord>(rec: R, proxy: object): object {
    captures.push(rec)
    return proxy
  }

  function coercePropName(name: unknown): string {
  ...
  const mockInstance = {
    ...
    // array `.map(...)`, etc.).
    getPropertyValue(_name: unknown) {
      return ''
    },
    hasCodeConnect() {
      return false
    ...
    findInstance(_layerName: unknown) {
      return mockInstance
    },
    findText(_layerName: unknown) {
    ...
    findConnectedInstance(_id: unknown) {
      return null
    },
    findConnectedInstances(_selectorFn: unknown) {
      return []
    },
    findLayers(_selectorFn: unknown) {
      return []
    },
    executeTemplate() {
      const r = Object.create(null)
  ...
  function makeFigmaCode(): (strings: readonly string[], ...values: unknown[]) => FigmaCodeResult {
    return (strings: readonly string[], ...values: unknown[]) => {
      const result = Object.create(null) as Record<string | symbol, unknown>
      result[FIGMA_CODE_MARKER] = true
      result.strings = strings
      result.values = values
      return result as unknown as FigmaCodeResult
    }
  }
  ...
  const propertiesV1 = {
    ...
    boolean(name: unknown, options?: unknown) {
      return mockInstance.getBoolean(name, options)
    },
    enum(name: unknown, options: unknown) {
    ...
    children(_layerNames: unknown) {
      return []
    },
  }
    ; (mockInstance as Record<string, unknown>).__properties__ = propertiesV1
  ...
  const figma = {
    ...
    value(raw: unknown, preview?: unknown) {
      const r = Object.create(null)
      r[FIGMA_VALUE_MARKER] = true
      r.type = 'value'
      r.value = raw
      r.preview = preview
      return r
    ...
    helpers: {
      react: {
        renderProp(name: unknown, prop: unknown): RenderPropMarker {
        ...
        renderChildren(prop: unknown) {
          return String(prop)
        },
        renderPropValue(prop: unknown) {
          return String(prop)
        },
        stringifyObject(o: unknown) {
          try {
        ...
        templateString(v: unknown) {
          return makeHelperWrapper('template-string', v)
        },
        reactComponent(v: unknown) {
  ...
  const jsxRuntime = {
    jsx(tag: unknown, props: unknown) {
      return makeJsxStub(tag, props)
    },
    jsxs(tag: unknown, props: unknown) {
      return makeJsxStub(tag, props)
    },
    jsxDEV(tag: unknown, props: unknown) {
      return makeJsxStub(tag, props)
    },
 ...
 */
function makeHelperWrapper(kind: string, value: unknown): Record<string, unknown> {
...
function makeJsxStub(tag: unknown, props: unknown): Record<string | symbol, unknown> {
  const r = Object.create(null) as Record<string | symbol, unknown>
  r[JSX_STUB_MARKER] = true
  r.tag = tag
  r.props = props
  return r
}
