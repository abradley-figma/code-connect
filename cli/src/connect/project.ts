...
import { normalizePath, normalizePathsToAbsolute } from '../common/path'
    ...
    (currentDir) => {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        if (packageJsonContains(packageJson, 'react')) {
          showParserMessage(
            `Using "react" parser as package.json containing a "react" ${packageJson.dependencies && packageJson.dependencies['react']
              ? 'dependency'
              : 'peer dependency'
...
export function determineLabelFromProject(dir: string): string | undefined {
  function showMessage(
    ...
    showParserMessage(
      `Using "${libraryName}" label as package.json containing a "${moduleName}" ${packageJson.dependencies && packageJson.dependencies[moduleName]
        ? 'dependency'
        : 'peer dependency'
      } was found in ${currentDir}`,
    )
 ...
 */
export function resolveTemplateGlobs(
  config: CodeConnectConfig,
  isTemplatesOnlyCLI = false,
): { include: string[] | undefined; exclude: string[] } {
  ...
  const include = config.include || defaultIncludeGlobs
 ...
 */
export async function discoverFilesByGlob(
  root: string,
  include: string[],
  exclude: string[],
): Promise<string[]> {
  const files = await glob(normalizePathsToAbsolute(root, include), {
    nodir: true,
    ignore: normalizePathsToAbsolute(root, exclude),
...
export async function getProjectInfoFromConfig(
  ...
  const { include, exclude } = resolveTemplateGlobs(config, isTemplatesOnlyCLI)

  if (config.parser === 'custom' && (!include || include.length === 0)) {
  ...
  const files = await discoverFilesByGlob(absPath, include, exclude)
 ...
 */
export function isTemplateFilePath(
  relPath: string,
  include: string[],
  exclude: string[],
): boolean {
  // Files outside the root are never considered "template files".
  // `relPath` is POSIX-shaped by contract (callers run it through
  // `normalizeRelativePath`), so use `posix.isAbsolute` to keep the
  // predicate platform-independent — `path.isAbsolute` is platform-aware
  // and would treat `'C:/abs'` differently on POSIX vs Windows.
  if (relPath.startsWith('..') || path.posix.isAbsolute(relPath)) return false
  const matchOpts = { dot: false, nocase: process.platform === 'win32' }
  const included = include.some((g) => minimatch(relPath, g, matchOpts))
  if (!included) return false
  const excluded = exclude.some((g) => minimatch(relPath, g, matchOpts))
  return !excluded
}
