...
exports.emitRuntimeModule = emitRuntimeModule;
exports.resolveRuntimeFilePath = resolveRuntimeFilePath;
exports.generateRuntimeShim = generateRuntimeShim;
 ...
 */
function generateManifest(componentDescriptors) {
    const manifest = {
 ...
 */
async function emitRuntimeModule(runtimeFilePath, manifest) {
    const nextRuntimeShim = generateRuntimeShim(manifest);
    let previousRuntimeShim;
    try {
        previousRuntimeShim = await (0, promises_1.readFile)(runtimeFilePath, 'utf8');
    }
    catch {
    ...
    await (0, promises_1.mkdir)(node_path_1.default.dirname(runtimeFilePath), { recursive: true });
    await (0, promises_1.writeFile)(runtimeFilePath, nextRuntimeShim, 'utf8');
    return true;
}
/**
 ...
 */
function resolveRuntimeFilePath(root, outFile) {
    if (outFile) {
        return (0, path_1.normalizeResolvePath)(root, outFile);
    }
    return (0, path_1.normalizeResolvePath)(root, 'node_modules', '.cache', 'figma-code-connect', 'runtime.js');
}
 ...
 */
function generateRuntimeShim(manifest) {
