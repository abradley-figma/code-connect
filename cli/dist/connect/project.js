...
exports.resolveTemplateGlobs = resolveTemplateGlobs;
exports.discoverFilesByGlob = discoverFilesByGlob;
exports.getProjectInfoFromConfig = getProjectInfoFromConfig;
exports.isTemplateFilePath = isTemplateFilePath;
 ...
 */
function resolveTemplateGlobs(config, isTemplatesOnlyCLI = false) {
 ...
 */
async function discoverFilesByGlob(root, include, exclude) {
    const files = await (0, glob_1.glob)((0, path_2.normalizePathsToAbsolute)(root, include), {
        nodir: true,
        ignore: (0, path_2.normalizePathsToAbsolute)(root, exclude),
    ...
    return [...new Set(files.map(path_2.normalizePath))].sort();
}
/**
 ...
 */
function isTemplateFilePath(relPath, include, exclude) {
    ...
    if (relPath.startsWith('..') || path_1.default.posix.isAbsolute(relPath))
        return false;
    const matchOpts = { dot: false, nocase: process.platform === 'win32' };
    const included = include.some((g) => (0, minimatch_1.minimatch)(relPath, g, matchOpts));
    if (!included)
        return false;
    const excluded = exclude.some((g) => (0, minimatch_1.minimatch)(relPath, g, matchOpts));
    return !excluded;
}
/**
