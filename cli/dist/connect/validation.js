...
async function fetchNodeInfo(baseApiUrl, fileKey, nodeIdsChunk, accessToken, useOAuth = false) {
    try {
        const response = await fetch_1.request.get(`${baseApiUrl}${fileKey}/nodes?ids=${nodeIdsChunk.join(',')}`, { headers: (0, figma_rest_api_1.getHeaders)(accessToken, useOAuth) });
...
async function validateDocs(cmd, accessToken, docs, apiUrlOverride, useOAuth = false) {
    let baseApiUrl = (0, figma_rest_api_1.getApiUrl)(docs?.[0]?.figmaNode ?? '', apiUrlOverride) + '/files/';
    ...
    for (let i = 0; i < fileKeys.length; i++) {
        ...
        for (let batch = 0; batch < chunks.length; batch++) {
            ...
            const nodeMapRet = await fetchNodeInfo(baseApiUrl, fileKey, nodeIdsChunk, accessToken, useOAuth);
            if (!nodeMapRet) {
                return false;
            }
            valid =
