...
makeIntrinsic('slot', (name) => {
    return {
        parse: (exp, ctx) => {
            const { sourceFile } = ctx;
            const figmaPropNameIdentifier = exp.arguments?.[0];
            (0, compiler_1.assertIsStringLiteral)(figmaPropNameIdentifier, sourceFile, `${name} takes a single argument which is the Figma property name`);
            return {
...
function valueToString(value, childLayer, isForMigration) {
    ...
    if ('kind' in value) {
        ...
        return `${intrinsicToString(value, childLayer, isForMigration)}`;
    }
...
function valueMappingToString(valueMapping, childLayer, isForMigration) {
...
function intrinsicToString({ kind, args, modifiers = [] }, childLayer, isForMigration) {
        ...
        case IntrinsicKind.NestedProps: {
            ...
            if (isForMigration) {
                ...
                body += `return {\n${Object.entries(args.props)
                    .map(([key, intrinsic]) => `${key}: ${nestedLayerRef}.type !== "ERROR" ? ${intrinsicToString(intrinsic, nestedLayerRef, isForMigration)} : undefined,\n`)
                    .join('')}\n        }\n`;
            }
            else {
                body += `return ${nestedLayerRef}.type === "ERROR" ? ${nestedLayerRef} : {
${Object.entries(args.props).map(([key, intrinsic]) => `${key}: ${intrinsicToString(intrinsic, nestedLayerRef, isForMigration)}\n`)}
        }\n`;
            }
            return `(function () {${body}})()`;
        }
