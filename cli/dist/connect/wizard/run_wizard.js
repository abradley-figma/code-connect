...
async function fetchTopLevelComponentsFromFile({ accessToken, figmaUrl, cmd, projectInfo, }) {
    // TODO enter create flow if node-id specified
    const fileKey = (0, helpers_1.parseFileKey)(figmaUrl);
    const apiUrl = (0, figma_rest_api_1.getApiUrl)(figmaUrl ?? '', cmd.apiUrl || projectInfo?.config.apiUrl) +
...
async function runWizard(cmd) {
    ...
    if (!componentsFromFile || componentsFromFile.length === 0) {
        logging_1.logger.info('No components found in the provided Figma file.');
        (0, helpers_1.exitWithFeedbackMessage)(1);
    }
    if (!hasConfigFile) {
        const { createConfigFile } = await askQuestionOrExit({
