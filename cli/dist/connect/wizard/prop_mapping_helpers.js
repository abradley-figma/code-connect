...
async function getEmbeddingsMatchResults({ propMappingData, accessToken, figmaUrl, mockResponseName, apiUrlOverride, }) {
    ...
    if (uniqueMatchableNames.length > 0) {
        const res = mockResponseName
            ...
            : await (0, embeddings_1.fetchEmbeddings)({ uniqueMatchableNames, accessToken, figmaUrl, apiUrlOverride });
...
async function generateAllPropsMappings({ propMappingData, accessToken, figmaUrl, useAi, mockResponseName, apiUrlOverride, }) {
    let allMatchResults = {};
    if (useAi) {
        try {
