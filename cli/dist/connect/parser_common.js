 ...
 */
function getReferencedPropsForTemplate({ propMappings = {}, isForMigration, }) {
    let templateCode = '';
    if (Object.keys(propMappings).length > 0) {
        for (const prop in propMappings) {
            const propMapping = propMappings[prop];
            templateCode += `const ${prop} = ${(0, intrinsics_1.valueToString)(propMapping, undefined, isForMigration)}\n`;
        }
