 ...
 */
async function convertStorybookFiles({ projectInfo, storiesGlob = '**/*.stories.{tsx,ts,jsx,js}', isForMigration = false, }) {
    ...
    return Promise.all(storyFiles.map((path) => convertStorybookFile({
        path,
        tsProgram,
        config,
        remoteUrl,
...
async function convertStorybookFile({ path, tsProgram, remoteUrl, config, absPath, isForMigration = false, }) {
    ...
    try {
        ...
        for (const statement of sourceFile.statements) {
            ...
            let render = (0, parser_1.parseJSXRenderFunction)(statementToParse, parserContext, exampleProps, isForMigration);
            if (!render) {
                continue;
            }
